...
class GetPrograms {
    ...
    async execute(request) {
        ...
        const cacheKey = cacheKeyBuilder_1.CacheKeyBuilder.forPrograms(normalized);
        ...
        const channelFilter = normalized.channels?.length
            ...
            : null;
        const channelMeta = await this.getChannelMeta(channelFilter, normalized.country, normalized.channelTypes);
        ...
        const dtoPrograms = this.layoutBuilder.buildProgramLayouts(sortedPrograms, normalized.date, this.timeSlots, normalized.timeSlot, normalized.fields);
        const response = {
        ...
        if (canUsePrecomputed) {
            await this.cacheRepository.set(`precomputed:programs:${normalized.date}:${normalized.fields}`, response, this.cacheTtlSeconds * 4);
        }
        return response;
    }
    async loadScheduleSnapshot(date, fields) {
        const cacheKey = `schedule:json:${date}:${fields}`;
        const cached = await this.cacheRepository.get(cacheKey);
        if (cached && (!cached.layoutVersion || cached.layoutVersion === this.layoutVersion)) {
            return { ...cached, fromCache: true };
        }
        const doc = (await Schedule_model_1.ScheduleModel.findOne({ date }).lean().exec());
        if (!doc)
            return null;
        if (doc.layoutVersion && doc.layoutVersion !== this.layoutVersion) {
            return null;
        }
        const storedFields = doc.meta?.fields;
        if (storedFields && storedFields !== fields) {
            return null;
        }
        const programs = doc.channels?.flatMap((ch) => ch.programs || []) || [];
        const snapshot = {
            date: doc.date,
            timeSlots: doc.timeSlots,
            channels: this.sortChannels(doc.channelMeta || []),
            programs: programs,
    ...
    filterSchedulePrograms(snapshot, request) {
        ...
        const filteredChannels = snapshot.channels.filter((c) => {
            if (channelFilter && !channelFilter.has(c.id))
                return false;
            if (typeFilter && c.type && !typeFilter.has(String(c.type).toUpperCase()))
                return false;
            if (countryFilter && c.country && c.country.toLowerCase() !== countryFilter)
                return false;
            if (countryFilter && !c.country)
                return false;
            return true;
        });
        ...
        return {
            ...
            programs: this.sortProgramLayouts(programs, this.buildChannelOrder(filteredChannels)),
            fromCache: snapshot.fromCache,
        };
    }
    normalizeRequest(request) {
        const date = dateUtils_1.DateUtils.parseDateAlias(request.date);
        return {
            date,
            ...
            channelTypes: request.channelTypes?.filter(Boolean).map((t) => t.toUpperCase()) ?? [
                'TDT',
                'CABLE',
                'MOVISTAR',
                'AUTONOMICO',
    ...
    filterPrograms(programs, request, channelFilter, allowedChannelIds) {
        let result = programs;
        if (channelFilter) {
        ...
        if (request.timeSlot) {
            result = ProgramFilter_1.ProgramFilter.byTimeSlot(result, request.timeSlot, ProgramFilter_1.DEFAULT_TIME_SLOTS);
        }
    ...
    buildChannelOrder(channels) {
        const order = new Map();
        channels.forEach((c, idx) => order.set(c.id, idx));
        return order;
    }
    sortProgramsRaw(programs, channelOrder) {
    ...
    sortProgramLayouts(programs, channelOrder) {
        return [...programs].sort((a, b) => {
            const oA = channelOrder.get(a.channelId) ?? 9999;
            const oB = channelOrder.get(b.channelId) ?? 9999;
            if (oA !== oB)
...
//# sourceMappingURL=GetPrograms.js.map