...
exports.ProgramDeduplicator = void 0;
...
class ProgramDeduplicator {
    constructor() {
        this.dedupeLogger = logger_1.logger.child('ProgramDeduplicator');
    }
    dedupe(programs) {
        ...
        grouped.forEach((bucket) => {
            ...
            const winner = bucket.reduce((best, candidate) => this.pickBetterProgram(best, candidate));
            cleaned.push(winner);
    ...
    pickBetterProgram(current, candidate) {
    ...
    static isGenericTitle(title) {
        ...
        if (genericRoots.includes(normalized))
            return true;
        if (/^(cine|pelicula|peliculas|programa|programacion|serie|series)\b/.test(normalized)) {
...
exports.ProgramDeduplicator = ProgramDeduplicator;
//# sourceMappingURL=ProgramDeduplicator.js.map