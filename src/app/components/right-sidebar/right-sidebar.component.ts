...
export class RightSidebarComponent implements OnInit {
  ...
  private readonly tvGuide = inject(TvGuideService);
  ...
  ngOnInit(): void {
    ...
    // If the featured lists are empty (app didn't start on Home), trigger load
    try {
      if (this.popular_movies().length === 0) {
        console.log('🔄 RIGHT-SIDEBAR - Triggering peliculas destacadas load');
        this.tvGuide.setPeliculasDestacadas();
      }
      if (this.popular_series().length === 0) {
        console.log('🔄 RIGHT-SIDEBAR - Triggering series destacadas load');
  ...
  private initializeDataStreams(): void {
    ...
    this.homeDataService.popularMovies$
      ...
      .subscribe((movies) => {
        console.log(
          '🎬 RIGHT-SIDEBAR - Popular movies received:',
          movies.length
        );
        ...
        if (movies && movies.length > 0) {
          ...
          console.log(
            '✅ RIGHT-SIDEBAR - Movies converted and set:',
            convertedMovies.length
          );
        } else {
          console.log(
            '⚠️ RIGHT-SIDEBAR - No movies received, keeping loading state'
          );
        }
      });
    ...
    this.tvGuide
      ...
      .subscribe((series) => {
        if (series && series.length > 0) {
          ...
          console.log(
            '📺 RIGHT-SIDEBAR - Series received and set:',
      ...
      .subscribe((loading) => {
        if (!loading && this.popular_movies().length === 0) {
          ...
          setTimeout(() => {
            if (this.popular_movies().length === 0) {
              console.log(
                '⏰ RIGHT-SIDEBAR - Loading timeout, no movies available'
              );
              this.isLoading.set(false);
            }
          }, 2000);
   ...
   */
  private convertToSidebarFormat(
    movies: any[],
    type: 'movie' | 'series'
  ): ISidebarItem[] {
    const maxItems = 4;
    ...
    return movies.slice(0, maxItems).map((movie, index) => {
      // movie.title puede ser string o un objeto { value }
      const rawTitle =
        typeof movie?.title === 'string'
          ? movie.title
          : movie?.title?.value || movie?.name || '';

      const baseTitle =
        rawTitle || `${type === 'movie' ? 'Película' : 'Serie'} ${index + 1}`;
      ...
      return {
        ...
        category: {
          value:
            type === 'series'
              ? movie.category || 'Series,Drama'
              : movie.category || 'Cine,Drama',
        },
  ...
  public navigateTo(item: ISidebarItem): void {
    ...
    console.log('🔗 RIGHT-SIDEBAR - Navigating to details:', rawTitle);

    // Obtener slug usando slugify para eliminar caracteres especiales
    const slug = slugify(rawTitle);

    // Heurística simple para distinguir película vs serie/programa:
    // - categoría que empieza por 'Cine' -> película
    // - o si tiene poster/tmdbId -> película
    const cat = item?.category?.value || '';
    const looksLikeMovie =
      cat.startsWith('Cine') || !!(item as any).tmdbId || !!(item as any).icon;
  ...
  public navigateTo2(type: string): void {
    ...
    if (type === 'movie') {
      // marcar estado en el servicio para que ListaDestacadas cargue películas
      try {
  ...
  public getMovieCategory(movie: ISidebarItem): string {
    ...
    else if (Array.isArray(val)) raw = val.join(',');
