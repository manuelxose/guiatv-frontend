...
export class FirebaseProgramProvider implements IProgramDataProvider {
  ...
  getPrograms(date: string): Observable<ITvProgram[] > {
    ...
    return this.http.get<FirebaseChannelData[]>(`${this.baseUrl}/programas/date/${date}`)
      .pipe(
  ...
  getChannels(): Observable<IChannel[]> {
    ...
    return this.http.get<IChannel[]>(`${this.baseUrl}/canales`)
      .pipe(
        tap(channels => {
   ...
   */
  private transformFirebaseData(channelDataArray: FirebaseChannelData[]): ITvProgram[] {
    if (!Array.isArray(channelDataArray)) {
      this.logger.warn('Invalid channel data received from Firebase', channelDataArray);
      return [];
    }

    this.logger.debug(`Transforming ${channelDataArray.length} channels from Firebase`);

    const allPrograms: ITvProgram[] = [];

    channelDataArray.forEach((channelData, channelIndex) => {
      if (!channelData || !channelData.channel) {
        this.logger.warn(`Invalid channel data at index ${channelIndex}`, channelData);
        return;
      }
    ...
    this.logger.info(`Transformed ${allPrograms.length} total programs from ${channelDataArray.length} channels`);
  ...
  private generateId(channelIndex: number, programIndex: number): string {
    return `program_${Date.now()}_${channelIndex}_${programIndex}_${Math.random().toString(36).substr(2, 9)}`;
  }
  ...
  getProgramsForProgramList(date: string): Observable<any[]> {
    this.logger.info(`Getting programs in ProgramList format for ${date}`);
    
    return this.http.get<FirebaseChannelData[]>(`${this.baseUrl}/programas/date/${date}`)
      .pipe(
        map(channelDataArray => this.transformForProgramList(channelDataArray)),
        tap(data => {
          this.logger.info(`ProgramList format data prepared`, { 
            channelsCount: data.length,
            totalPrograms: data.reduce((sum, ch) => sum + (ch.channels?.length || 0), 0)
          });
        }),
        catchError(error => {
          this.logger.error(`Failed to fetch programs for ProgramList format`, error);
          throw error;
        })
      );
  }
   ...
   */
  private transformForProgramList(channelDataArray: FirebaseChannelData[]): any[] {
    if (!Array.isArray(channelDataArray)) {
    ...
    return channelDataArray.map(channelData => ({
      ...
      channels: channelData.programs.map(program => ({
        ...
        duracion: this.calculateDuration(program.start, program.end || program.stop),
        start: program.start,
