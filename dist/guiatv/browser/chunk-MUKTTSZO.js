...
// src/app/services/core/config.service.ts
var _AppConfigurationService = class _AppConfigurationService {
  constructor(platformId) {
    this.platformId = platformId;
    const isBrowser = isPlatformBrowser(this.platformId);
    const envBaseUrl = environment.API_BASE_URL?.trim();
  ...
  updateConfig(newConfig) {
    this.config = __spreadValues(__spreadValues({}, this.config), this.mergeDeep(this.config, newConfig));
  }
  getApiConfig() {
...
_AppConfigurationService.\u0275fac = function AppConfigurationService_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _AppConfigurationService)(\u0275\u0275inject(PLATFORM_ID));
};
_AppConfigurationService.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _AppConfigurationService, factory: _AppConfigurationService.\u0275fac, providedIn: "root" });
var AppConfigurationService = _AppConfigurationService;
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(AppConfigurationService, [{
    type: Injectable,
    args: [{
      providedIn: "root"
    }]
...
var _HttpService = class _HttpService {
  constructor(http, configService) {
    ...
    this.today_programs = [];
    this.tomorrow_programs = [];
    this.after_tomorrow_programs = [];
    this.headers = new HttpHeaders({
      Accept: "application/json",
      Authorization: "Bearer your-token"
    });
    this.programasSource = new BehaviorSubject([]);
    this.programas$ = this.programasSource.asObservable();
  ...
  setProgramasByDay(programas, dia) {
    switch (dia) {
      case "today":
  ...
  getProgramacion(dia) {
    ...
    if (programas && programas.length > 0) {
      console.log(`\u{1F4BE} CACHE - Usando datos en cache para ${dia} (NO se llama a la base de datos)`);
      return of(programas);
    }
    if (_HttpService.isLoadingData[dia]) {
    ...
    _HttpService.isLoadingData[dia] = true;
    _HttpService.databaseCallCounter++;
    const baseUrl = this.configService.getApiConfig().backend.baseUrl;
    const apiUrl = `${baseUrl}/programs?date=${dia}`;
    return this.http.get(apiUrl).pipe(map((response) => {
      const programs = response?.data?.programs ?? response?.programs ?? (Array.isArray(response) ? response : []);
      return Array.isArray(programs) ? programs : [];
    }), tap((data) => {
      console.log(`\u2705 DATABASE RESPONSE #${_HttpService.databaseCallCounter} - Recibidos ${data.length} programas para ${dia}`);
      this.setProgramasByDay(data, dia);
      _HttpService.isLoadingData[dia] = false;
    }), catchError((error) => {
  ...
  setToLocalStorage(key, value) {
    const maxCacheAge = 1e3 * 60 * 60 * 24 * 1;
    localStorage.setItem(key, JSON.stringify(value));
    const currentTime = (/* @__PURE__ */ new Date()).getTime();
    localStorage.setItem(`${key}-time`, currentTime.toString());
    localStorage.setItem(`${key}-expiry`, (currentTime + maxCacheAge).toString());
  }
  setProgramas(programas, date) {
    return __async(this, null, function* () {
      console.log("\u{1F504} HTTP SERVICE - setProgramas llamado");
      console.log("\u{1F50D} HTTP SERVICE - Programas recibidos:", programas?.length || 0);
      console.log("\u{1F50D} HTTP SERVICE - Fecha:", date);
      console.log("\u{1F50D} HTTP SERVICE - isUpdating actual:", this.isUpdating);
      console.log("\u{1F50D} HTTP SERVICE - Estado actual del observable:", this.programasSource.value?.length || 0);
      if (this.isUpdating) {
        console.log("\u26A0\uFE0F Ya se est\xE1 actualizando, evitando emisi\xF3n duplicada");
        return;
      }
      ...
      console.log("\u{1F50D} HTTP SERVICE - Datos son diferentes:", isDataDifferent);
      console.log("\u{1F50D} HTTP SERVICE - Valor actual:", currentValue?.length || 0);
      console.log("\u{1F50D} HTTP SERVICE - Nuevo valor:", programas?.length || 0);
      if (isDataDifferent) {
        console.log("\u2705 HTTP SERVICE - Emitiendo nuevos datos al observable");
        this.programasSource.next(programas);
        console.log("\u{1F4E1} HTTP SERVICE - Datos emitidos. Nuevos suscriptores recibir\xE1n:", programas?.length || 0, "programas");
      } else {
        console.log("\u{1F504} HTTP SERVICE - Datos id\xE9nticos, no se emite");
      }
      setTimeout(() => {
        this.isUpdating = false;
        console.log("\u{1F513} HTTP SERVICE - Flag isUpdating reseteado");
      }, 100);
    });
  }
  getProgramas() {
  ...
  getPerson(person) {
    ...
    const options = {
      headers: new HttpHeaders({
        Accept: "application/json",
        Authorization: "Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJiNmE2MGE5YmRkZmZhZmU1YmMzZjZmNzAwZjIxZDBiMyIsInN1YiI6IjY1OGZmOWJlNDFhNTYxNjY3NTA0NzhmMCIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.A6Pj5IuTllkQRXivh_KMmlHrKAnkh6NvJTiaEPYBAO8"
      })
    };
  ...
  getMovieId(movie) {
    ...
    return this.http.get(url, options).pipe(catchError((error) => {
      console.error(`\u{1F6A8} SSL/HTTPS Error al obtener pel\xEDcula "${movie}":`, error.message);
      console.warn(`\u26A0\uFE0F Devolviendo respuesta vac\xEDa para evitar bloqueo de la aplicaci\xF3n`);
      return of({ results: [] });
    }));
  }
  getSeriesId(serie) {
    ...
    return this.http.get(url, options).pipe(catchError((error) => {
      console.error(`\u{1F6A8} SSL/HTTPS Error al obtener serie "${serie}":`, error.message);
      console.warn(`\u26A0\uFE0F Devolviendo respuesta vac\xEDa para evitar bloqueo de la aplicaci\xF3n`);
      return of({ results: [] });
    }));
  }
  ...
  getPopularSeries() {
    ...
    return this.http.get(url, options).pipe(catchError((error) => {
      console.error(`\u{1F6A8} SSL/HTTPS Error al obtener series populares:`, error.message);
      console.warn(`\u26A0\uFE0F Devolviendo respuesta vac\xEDa para evitar bloqueo de la aplicaci\xF3n`);
      return of({ results: [] });
    }));
  }
  ...
  getPopularMovies() {
    ...
    return this.http.get(url, options).pipe(catchError((error) => {
      console.error(`\u{1F6A8} SSL/HTTPS Error al obtener pel\xEDculas populares:`, error.message);
      console.warn(`\u26A0\uFE0F Devolviendo respuesta vac\xEDa para evitar bloqueo de la aplicaci\xF3n`);
      return of({ results: [] });
    }));
  }
   ...
   */
  getDatabaseCallSummary() {
    console.log(`\u{1F4CA} RESUMEN DE LLAMADAS A LA BASE DE DATOS:`);
    console.log(`\u{1F525} Total de llamadas a la base de datos (Firebase): ${_HttpService.databaseCallCounter}`);
  }
  /**
   * Reinicia el contador de llamadas a la base de datos
   */
  resetDatabaseCallCounter() {
    _HttpService.databaseCallCounter = 0;
    console.log(`\u{1F504} Contador de llamadas a la base de datos reiniciado`);
  }
};
_HttpService.databaseCallCounter = 0;
_HttpService.isLoadingData = {};
_HttpService.globalDataLoaded = false;
_HttpService.globalDataLoading = false;
_HttpService.\u0275fac = function HttpService_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _HttpService)(\u0275\u0275inject(HttpClient), \u0275\u0275inject(AppConfigurationService));
};
...
(() => {
  ...
  }], () => [{ type: HttpClient }, { type: AppConfigurationService }], null);
})();

// src/app/utils/utils.ts
function diffHour(start_string, stop_string) {
  const start = new Date(start_string);
  const stop = new Date(stop_string);
  const diffMs = stop.getTime() - start.getTime();
  const diffHrs = Math.floor(diffMs / 1e3 / 60 / 60);
  const diffMins = Math.floor(diffMs / 1e3 / 60 % 60);
...
function getHoraInicio(inicio) {
  return formatCorrectTime(inicio);
}
function isLive(dateIni, dateFin) {
...
function formatCorrectTime(dateString) {
...
function debugTimeZone(dateString, context = "") {
  const originalDate = new Date(dateString);
  const correctedDate = getCorrectTime(dateString);
  console.log(`\u{1F550} DEBUG TIMEZONE ${context}:`);
...
// src/app/config/di-tokens.ts
var PROGRAM_PROVIDER_TOKEN = new InjectionToken("ProgramProvider");
var MOVIE_PROVIDER_TOKEN = new InjectionToken("MovieProvider");
var CONTENT_FILTER_TOKEN = new InjectionToken("ContentFilter");
var DATA_TRANSFORMER_TOKEN = new InjectionToken("DataTransformer");
var INITIALIZATION_MANAGER_TOKEN = new InjectionToken("InitializationManager");
var CACHE_MANAGER_TOKEN = new InjectionToken("CacheManager");
var LOGGER_TOKEN = new InjectionToken("Logger");
var TIME_MANAGER_TOKEN = new InjectionToken("TIME_MANAGER_TOKEN");
var POSTER_PROVIDER_TOKEN = new InjectionToken("PosterProvider");

// src/app/interfaces/providers.interface.ts
var IProgramDataProvider = class {
};
var IMovieProvider = class {
};
var IContentFilter = class {
};
var IDataTransformer = class {
};
var IInitializationManager = class {
};
var ICacheManager = class {
};
var ILogger = class {
};
var IPosterProvider = class {
};

// src/app/interfaces/types.ts
var ContentType;
(function(ContentType2) {
  ContentType2["MOVIE"] = "Cine";
  ContentType2["SERIES"] = "Series";
  ContentType2["DOCUMENTARY"] = "Documental";
  ContentType2["NEWS"] = "Noticias";
  ContentType2["SPORTS"] = "Deportes";
})(ContentType || (ContentType = {}));
var InitializationStatus;
(function(InitializationStatus2) {
  InitializationStatus2["NOT_STARTED"] = "not_started";
  InitializationStatus2["IN_PROGRESS"] = "in_progress";
  InitializationStatus2["COMPLETED"] = "completed";
  InitializationStatus2["FAILED"] = "failed";
})(InitializationStatus || (InitializationStatus = {}));
var CacheKeys;
(function(CacheKeys2) {
  CacheKeys2["TODAY_PROGRAMS"] = "today_programs";
  CacheKeys2["TOMORROW_PROGRAMS"] = "tomorrow_programs";
  CacheKeys2["FEATURED_MOVIES"] = "featured_movies";
  CacheKeys2["POPULAR_SERIES"] = "popular_series";
})(CacheKeys || (CacheKeys = {}));

// src/app/services/features/featured-movies.service.ts
var _FeaturedMoviesService = class _FeaturedMoviesService {
  constructor(programProvider, movieProvider, contentFilter, dataTransformer, cache, logger) {
   ...
   */
  getFeaturedMoviesFromPrograms(programs) {
    try {
      ...
      if (featuredMoviePrograms.length === 0) {
        ...
        fallbackPrograms.forEach((program) => {
          ...
          const startTime = program.start ? new Date(program.start).toLocaleTimeString("es-ES", {
            hour: "2-digit",
            minute: "2-digit"
          }) : "Unknown";
   ...
   */
  processMoviePrograms(moviePrograms) {
    return this.dataTransformer.transformToFeaturedMovies(moviePrograms).pipe(map((featuredMovies) => {
   ...
   */
  getFeaturedMoviesFromTMDb() {
    this.logger.info("Fetching featured movies from TMDb as fallback");
    return this.movieProvider.getPopularMovies().pipe(map((movies) => {
   ...
   */
  enrichMoviesWithTMDbData(movies) {
    ...
    const enrichmentObservables = moviesToEnrich.map((movie) => this.movieProvider.searchMovie(movie.title).pipe(map((response) => {
      if (response?.results?.[0]) {
        const tmdbMovie = response.results[0];
        return __spreadProps(__spreadValues({}, movie), {
          rating: tmdbMovie.vote_average || movie.rating,
          description: tmdbMovie.overview || movie.description,
          poster: tmdbMovie.poster_path ? `https://image.tmdb.org/t/p/w500${tmdbMovie.poster_path}` : movie.poster,
          tmdbId: tmdbMovie.id,
    ...
    return forkJoin(enrichmentObservables).pipe(map((enrichedMovies) => {
   ...
   */
  getFeaturedMoviesHybrid(programs) {
  ...
  getFeaturedAndPopularMovies(programs) {
    return this.getFeaturedMoviesFromPrograms(programs).pipe(switchMap((res) => {
      if (res.success && Array.isArray(res.data) && res.data.length > 0) {
        const movies = res.data;
        const featured = this.selectFeaturedMovie(movies);
        const popular = movies.filter((m) => m !== featured).slice(0, 5);
        return this.enrichMoviesWithTMDbData(popular).pipe(map((enriched) => ({
          featured: featured || null,
          popular: enriched
        })));
      }
      return this.getFeaturedMoviesFromTMDb().pipe(switchMap((tmdbRes) => {
        if (tmdbRes.success && Array.isArray(tmdbRes.data) && tmdbRes.data.length > 0) {
...
_FeaturedMoviesService.\u0275fac = function FeaturedMoviesService_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _FeaturedMoviesService)(\u0275\u0275inject(PROGRAM_PROVIDER_TOKEN), \u0275\u0275inject(MOVIE_PROVIDER_TOKEN), \u0275\u0275inject(CONTENT_FILTER_TOKEN), \u0275\u0275inject(DATA_TRANSFORMER_TOKEN), \u0275\u0275inject(CACHE_MANAGER_TOKEN), \u0275\u0275inject(LOGGER_TOKEN));
};
_FeaturedMoviesService.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _FeaturedMoviesService, factory: _FeaturedMoviesService.\u0275fac, providedIn: "root" });
var FeaturedMoviesService = _FeaturedMoviesService;
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(FeaturedMoviesService, [{
    type: Injectable,
    args: [{
      providedIn: "root"
    }]
  }], () => [{ type: IProgramDataProvider, decorators: [{
    type: Inject,
    args: [PROGRAM_PROVIDER_TOKEN]
  }] }, { type: IMovieProvider, decorators: [{
    type: Inject,
    args: [MOVIE_PROVIDER_TOKEN]
  }] }, { type: IContentFilter, decorators: [{
    type: Inject,
    args: [CONTENT_FILTER_TOKEN]
  }] }, { type: IDataTransformer, decorators: [{
    type: Inject,
    args: [DATA_TRANSFORMER_TOKEN]
  }] }, { type: ICacheManager, decorators: [{
    type: Inject,
    args: [CACHE_MANAGER_TOKEN]
...
var _HomeDataService = class _HomeDataService {
  constructor(programProvider, initManager, logger, featuredMoviesService) {
    this.programProvider = programProvider;
    this.initManager = initManager;
    this.logger = logger;
    this.featuredMoviesService = featuredMoviesService;
    this.programsSubject = new BehaviorSubject([]);
    this.featuredMovieSubject = new BehaviorSubject(null);
    ...
    this.programListDataSubject = new BehaviorSubject([]);
    this.programs$ = this.programsSubject.asObservable();
    this.featuredMovie$ = this.featuredMovieSubject.asObservable();
    this.popularMovies$ = this.popularMoviesSubject.asObservable();
    this.loading$ = this.loadingSubject.asObservable();
    this.error$ = this.errorSubject.asObservable();
    this.programListData$ = this.programListDataSubject.asObservable();
   ...
   */
  initializeData() {
    this.logger.info("\u{1F680} Starting home data initialization");
    ...
    const standardPrograms$ = this.programProvider.getPrograms("today").pipe(
      timeout(45e3),
      // backend puede tardar precomputando, ampliamos margen
      catchError((err) => {
        this.logger.error("Timeout o error cargando programas est\xE1ndar:", err);
        throw err;
      })
    );
    const programListData$ = this.getProgramListData("today").pipe(
      timeout(45e3),
      // ampliamos margen para layouts
      catchError((err) => {
        this.logger.error("Timeout o error cargando datos de ProgramList:", err);
        throw err;
      })
    );
    return forkJoin({
      ...
      tap(({ programs, programListData }) => {
        this.logger.info(`\u2705 Data loaded - Programs: ${programs.length}, ProgramList channels: ${programListData.length}`);
      }),
      switchMap(({ programs, programListData }) => {
        ...
        this.logger.debug("\u{1F4CA} Data set in subjects, processing featured movies...");
        return this.processFeaturedMovies(programs);
      }),
      map(() => {
        this.logger.info("\u{1F3AC} Featured movies processed");
        const hasData = this.hasValidData();
        this.logger.info(`\u{1F4DD} Validating data: hasData = ${hasData}`);
      ...
      catchError((error) => {
        ...
        this.logger.error("\u274C Home data initialization failed:", errorMessage, error);
      ...
      finalize(() => {
        this.logger.info("\u{1F3C1} Initialize data finalized");
        setTimeout(() => {
          if (this.loadingSubject.value) {
            this.logger.warn("\u26A0\uFE0F Loading a\xFAn true despu\xE9s de finalize, forzando false");
            this.setLoadingState(false);
          }
        }, 1e3);
      })
  ...
  watchLoadingTimeout() {
    this.loadingSubject.subscribe((loading) => {
      if (loading) {
        ...
        this.loadingWatchTimerSub = timer(2e4).subscribe(() => {
          if (this.loadingSubject.value) {
            this.logger.error("\u{1F6A8} GLOBAL TIMEOUT: 20 segundos de loading, forzando false");
            this.setLoadingState(false);
            if (!this.hasValidData()) {
              this.setError("Timeout de carga. Por favor, recarga la p\xE1gina.");
            }
          }
        });
      } else {
  ...
  verifyDataState() {
    ...
    this.logger.debug("\u{1F50D} VERIFY DATA STATE:");
    ...
    if (state.isLoading && state.hasData) {
      ...
      this.logger.warn("\u{1F527} Corrigiendo...");
      this.setLoadingState(false);
    }
   ...
   */
  processFeaturedMovies(programs) {
    return this.featuredMoviesService.getFeaturedAndPopularMovies(programs).pipe(tap(({ featured, popular }) => {
      this.logger.debug(`Featured movies processed: ${featured ? 1 : 0} featured, ${popular.length} popular`);
   ...
   */
  getProgramListData(date) {
    const apiProvider = this.programProvider;
    if (apiProvider.getProgramsForProgramList) {
      return apiProvider.getProgramsForProgramList(date);
    } else {
      return this.programProvider.getPrograms(date).pipe(map((programs) => this.convertProgramsToListFormat(programs)));
    }
  }
  /**
   * Convierte programas estándar al formato que espera ProgramListComponent - SIN CAMBIOS
   */
  convertProgramsToListFormat(programs) {
   ...
   */
  loadDataForDay(dayIndex) {
    this.logger.info(`Loading data for day ${dayIndex}`);
    this.setLoadingState(true);
    this.clearError();
    const dateParam = this.getDayParam(dayIndex);
    const standardPrograms$ = this.programProvider.getPrograms(dateParam).pipe(timeout(45e3), catchError((err) => {
      this.logger.error(`Error loading programs for day ${dayIndex}:`, err);
      throw err;
    }));
    const programListData$ = this.getProgramListData(dateParam).pipe(timeout(45e3), catchError((err) => {
      this.logger.error(`Error loading program list for day ${dayIndex}:`, err);
      throw err;
    }));
    return forkJoin({
      programs: standardPrograms$,
      ...
      tap(({ programs, programListData }) => {
        this.logger.info(`Day ${dayIndex} data loaded - Programs: ${programs.length}, Channels: ${programListData.length}`);
      }),
      switchMap(({ programs, programListData }) => {
        this.programsSubject.next(programs);
        this.programListDataSubject.next(programListData);
      ...
      finalize(() => {
        this.logger.debug(`\u{1F3C1} Load day ${dayIndex} finalized`);
        if (this.loadingSubject.value) {
          this.logger.warn("\u26A0\uFE0F Loading a\xFAn true en finalize (loadDataForDay), forzando false ahora");
          this.setLoadingState(false);
        }
        setTimeout(() => {
          if (this.loadingSubject.value) {
            this.logger.warn("\u26A0\uFE0F Loading a\xFAn true despu\xE9s de finalize (loadDataForDay), forzando false");
            this.setLoadingState(false);
          }
        }, 500);
      })
   ...
   */
  refreshData() {
    this.logger.info("\u{1F504} Forcing data refresh");
    this.resetData();
  ...
  setLoadingState(loading) {
    ...
    if (previousState !== loading) {
      this.logger.debug(`\u{1F504} Loading state change: ${previousState} -> ${loading}`);
    }
...
_HomeDataService.\u0275fac = function HomeDataService_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _HomeDataService)(\u0275\u0275inject(PROGRAM_PROVIDER_TOKEN), \u0275\u0275inject(INITIALIZATION_MANAGER_TOKEN), \u0275\u0275inject(LOGGER_TOKEN), \u0275\u0275inject(FeaturedMoviesService));
};
_HomeDataService.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _HomeDataService, factory: _HomeDataService.\u0275fac, providedIn: "root" });
var HomeDataService = _HomeDataService;
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(HomeDataService, [{
    type: Injectable,
    args: [{
      providedIn: "root"
    }]
    ...
    type: Inject,
    args: [PROGRAM_PROVIDER_TOKEN]
  }] }, { type: IInitializationManager, decorators: [{
    type: Inject,
    args: [INITIALIZATION_MANAGER_TOKEN]
  }] }, { type: ILogger, decorators: [{
    type: Inject,
    args: [LOGGER_TOKEN]
  }] }, { type: FeaturedMoviesService }], null);
})();

// src/app/services/tv-guide.service.ts
var _TvGuideService = class _TvGuideService {
  constructor(http, homeDataService) {
    this.http = http;
    ...
    this.isMovies = false;
    this.peliculasDestacadasSource = new BehaviorSubject([]);
    this.peliculasDestacadas$ = this.peliculasDestacadasSource.asObservable();
    this.seriesDestacadasSource = new BehaviorSubject([]);
    this.seriesDestacadas$ = this.seriesDestacadasSource.asObservable();
    this.detallesProgramaSource = new BehaviorSubject([]);
    this.detallesPrograma$ = this.detallesProgramaSource.asObservable();
    try {
      if (this.homeDataService && this.homeDataService.getProgramListData$) {
        this.homeDataService.getProgramListData$().subscribe((data) => {
          if (Array.isArray(data) && data.length > 0) {
            console.log("\u{1F501} TvGuideService - syncing programListData to HttpService.programas$ (count=", data.length, ")");
            try {
              this.http.setProgramas(data, "today");
            } catch (err) {
              console.warn("\u26A0\uFE0F TvGuideService - failed to set programas in HttpService", err);
            }
          }
        });
      }
    } catch (err) {
      console.warn("\u26A0\uFE0F TvGuideService init: could not subscribe to HomeDataService", err);
    }
  }
  // GESTION DE LOS DETALLES DE LOS PROGRMAS
  setDetallesPrograma(programa) {
    return __async(this, null, function* () {
  ...
  setPeliculasDestacadas() {
    return __async(this, null, function* () {
      if (_TvGuideService.moviesCacheLoaded && _TvGuideService.moviesCache.length > 0) {
        console.log(`\u{1F4BE} CACHE (Movies) - Usando datos en cache de pel\xEDculas destacadas (NO se llama a TMDb API)`);
        console.log("\u{1F4BE} Emitting peliculas desde cache, count=", _TvGuideService.moviesCache.length);
        this.peliculasDestacadasSource.next(_TvGuideService.moviesCache);
        return;
      }
      if (_TvGuideService.isLoadingMovies) {
        console.log(`\u23F3 WAITING (Movies) - Ya hay una carga de pel\xEDculas en progreso, esperando...`);
        return;
      }
      _TvGuideService.isLoadingMovies = true;
      console.log(`\u{1F3AC} LOADING (Movies) - Iniciando carga de pel\xEDculas destacadas`);
      try {
        const peliculas = yield this.getBestRatedMovies();
        _TvGuideService.moviesCache = peliculas;
        _TvGuideService.moviesCacheLoaded = true;
        console.log("\u{1F4BE} Saving peliculas to cache, count=", peliculas.length);
        this.peliculasDestacadasSource.next(peliculas);
        console.log(`\u2705 CACHE SAVED (Movies) - ${peliculas.length} pel\xEDculas guardadas en cache`);
      } catch (error) {
        console.error(`\u274C ERROR (Movies) - Error cargando pel\xEDculas:`, error);
      } finally {
        _TvGuideService.isLoadingMovies = false;
      }
    });
  }
  getPeliculasDestacadas() {
    console.log("\u{1F4E1} getPeliculasDestacadas called - returning observable");
  ...
  setSeriesDestacadas() {
    return __async(this, null, function* () {
      if (_TvGuideService.seriesCacheLoaded && _TvGuideService.seriesCache.length > 0) {
        console.log(`\u{1F4BE} CACHE (Series) - Usando datos en cache de series destacadas (NO se llama a TMDb API)`);
        console.log("\u{1F4BE} Emitting series desde cache, count=", _TvGuideService.seriesCache.length);
        this.seriesDestacadasSource.next(_TvGuideService.seriesCache);
        return;
      }
      if (_TvGuideService.isLoadingSeries) {
        console.log(`\u23F3 WAITING (Series) - Ya hay una carga de series en progreso, esperando...`);
        return;
      }
      _TvGuideService.isLoadingSeries = true;
      console.log(`\u{1F4FA} LOADING (Series) - Iniciando carga de series destacadas`);
      try {
        const series = yield this.getBestRatedSeries();
        _TvGuideService.seriesCache = series;
        _TvGuideService.seriesCacheLoaded = true;
        console.log("\u{1F4BE} Saving series to cache, count=", series.length);
        ...
        _TvGuideService.isLoadingSeries = false;
      }
    });
  }
  getSeriesDestacadas() {
    console.log("\u{1F4E1} getSeriesDestacadas called - returning observable");
    ...
    return programs.flatMap((data) => {
      try {
        ...
        if (Array.isArray(data.channel))
          return data.channel.filter(Boolean);
  ...
  extractPrograms(programs) {
    ...
    return programs.flatMap((data) => {
      try {
        if (!data)
          return [];
        return Array.isArray(data.programs) ? data.programs.filter(Boolean) : [];
      } catch (err) {
        return [];
      }
    });
  ...
  getAllMovies() {
    const peliculas = this.programsByCategory("Cine").filter((programa) => programa?.desc?.details !== "Emisi\xF3n de una pel\xEDcula.").sort((a, b) => new Date(a.start).getTime() - new Date(b.start).getTime());
    return peliculas;
  }
  getMoviesByChannel(channelId) {
    return this.programsByChannel(channelId, "Cine").filter((programa) => programa?.title?.value !== "Cine");
  }
  getMoviesByDate(date) {
    return this.programsByDate(date, "Cine").filter((programa) => programa?.title?.value !== "Cine");
  }
  ...
  getProgramsByCategory(category, channel) {
    return this.getAllProgramsByCategory(category, channel);
  }
  getBestRatedMovies() {
    return __async(this, null, function* () {
      const movies = this.getAllMovies();
      const moviesAfter22 = movies.filter((movie) => getHoraInicio(movie.start) > "22:00" && movie.starRating !== null && movie.title.value !== "Cine" && //no puede ser una serie el titulo no puede tener T seguido de un numero ejemplo T1 o T2 o T3 emplea expresiones regulares
      !movie.title.value.match(/T\d/));
      console.log(`\u{1F3AC} TMDb API - Preparando ${moviesAfter22.length} llamadas para obtener ratings de pel\xEDculas`);
      const ratings = yield Promise.all(moviesAfter22.map((movie, index) => __async(this, null, function* () {
        _TvGuideService.externalApiCallCounter++;
        console.log(`\u{1F525} TMDb API CALL #${_TvGuideService.externalApiCallCounter} - Obteniendo rating para: ${movie.title.value}`);
        const response = yield lastValueFrom(this.http.getMovieId(movie.title.value));
        console.log(`\u2705 TMDb API RESPONSE #${_TvGuideService.externalApiCallCounter} - Rating obtenido para: ${movie.title.value}`);
        return response.results[0];
      })));
      console.log(`\u{1F4CA} TMDb API - TOTAL de llamadas realizadas hasta ahora: ${_TvGuideService.externalApiCallCounter}`);
      const moviesWithRatings = moviesAfter22.map((movie) => {
        const ratingData = ratings.find((rating2) => {
          if (!rating2) {
            return false;
          }
          return rating2.title.toLowerCase() === movie.title.value.toLowerCase();
        });
        const rating = ratingData?.vote_average ?? null;
        return __spreadProps(__spreadValues({}, movie), { starRating: rating });
      });
      const moviesOrdered = [...moviesWithRatings].sort((a, b) => b.starRating - a.starRating);
  ...
  getBestRatedSeries() {
    return __async(this, null, function* () {
      ...
      let seriesAfter22 = series.filter((serie) => {
        return getHoraInicio(serie.start) > "22:00" && serie.starRating !== null && serie.title.value !== "Cine";
      });
      seriesAfter22 = seriesAfter22.filter((serie, index, self) => {
        const title = serie.title.value.replace(/T\d+.*/, "");
        return index === self.findIndex((s) => s.title.value.replace(/T\d+.*/, "") === title);
      });
      console.log(`\u{1F4FA} TMDb API (Series) - Preparando ${seriesAfter22.length} llamadas para obtener ratings de series`);
      const ratings = yield Promise.all(seriesAfter22.map((serie) => __async(this, null, function* () {
        _TvGuideService.externalApiCallCounter++;
        const serieTitle = serie.title.value.replace(/T\d+.*/, "");
        console.log(`\u{1F525} TMDb API CALL #${_TvGuideService.externalApiCallCounter} - Obteniendo rating para serie: ${serieTitle}`);
        const response = yield lastValueFrom(this.http.getSeriesId(serieTitle));
        console.log(`\u2705 TMDb API RESPONSE #${_TvGuideService.externalApiCallCounter} - Rating obtenido para serie: ${serieTitle}`);
        return response.results[0];
      })));
      console.log(`\u{1F4CA} TMDb API (Series) - TOTAL de llamadas realizadas hasta ahora: ${_TvGuideService.externalApiCallCounter}`);
      const seriesWithRatings = seriesAfter22.map((serie) => {
        const ratingData = ratings.find((rating2) => {
          if (!rating2) {
            return false;
          }
          return rating2.name.toLowerCase() === serie.title.value.replace(/T\d+.*/, "").toLowerCase();
        });
        const rating = ratingData?.vote_average ?? null;
        return __spreadProps(__spreadValues({}, serie), { starRating: rating });
      });
      const seriesOrdered = [...seriesWithRatings].sort((a, b) => b.starRating - a.starRating);
  ...
  getSeriesByChannel(channelId) {
    return this.programsByChannel(channelId, "Series");
  }
  getSeriesByDate(date) {
  ...
  getProgramsByChannel(channelName) {
    return this.getProgramsByChannelName(channelName);
  }
  getAllCategories() {
    return this.getMainCategories();
  }
  getChannelCategories(progrmas) {
    return this.getCategoriesFromChannel(progrmas);
  }
  // Métodos genéricos para filtrar y obtener categorías únicas
  programsByCategory(categoryType, category) {
    const peliculas = this.getListaProgramas().filter((program) => {
      if (program?.category?.value?.split(",")[0] === categoryType && program?.desc?.details !== "Emisi\xF3n de una pel\xEDcula.")
        return program;
    }).sort((a, b) => new Date(a.start).getTime() - new Date(b.start).getTime());
    return peliculas;
  }
  programsByChannel(channelId, categoryType) {
    return (this.getListaCanales() || []).filter((programa) => {
      try {
        const cat = programa?.category?.value?.split(",")[0];
        const channelIdMatch = programa?.channel?.id === channelId || programa?.channel === channelId || programa?.id === channelId;
  ...
  programsByDate(date, categoryType) {
    return (this.getListaCanales() || []).flatMap((canal) => canal?.programs || []).filter((programa) => {
      try {
        return programa?.category?.value?.split(",")[0] === categoryType && programa?.start?.split("T")[0] === date;
      } catch (err) {
        return false;
      }
    });
  }
  getUniqueCategories(categoryType) {
    return this.getListaProgramas().filter((programa) => programa?.category?.value?.split(",")[0] === categoryType && programa?.desc?.details !== "Emisi\xF3n de una pel\xEDcula.").map((programa) => programa?.category?.value?.split(",")[1]).filter((value, index, self) => self.indexOf(value) === index);
  }
  getMoviesByCategories(maincat, categoryType) {
    return this.getListaProgramas().filter((programa) => {
      if (programa?.category?.value?.split(",")[0] === maincat && programa?.category?.value?.split(",")[1] === categoryType && programa?.desc?.details !== "Emisi\xF3n de una pel\xEDcula.")
        return programa;
    });
  }
  getSeriesByCategories(maincat, categoryType) {
    return this.getListaProgramas().filter((programa) => {
      if (programa?.category?.value?.split(",")[0] === maincat && programa?.category?.value?.split(",")[1] === categoryType)
        return programa;
    });
  }
  getCategoriesFromChannel(programs) {
    return programs.map((programa) => programa?.category?.value?.split(",")[1]).filter((value, index, self) => self.indexOf(value) === index);
  }
  getMainCategories() {
    return this.getListaProgramas().filter((programa) => programa?.category?.value?.split(",")[0]).map((programa) => programa?.category?.value?.split(",")[0]).filter((value, index, self) => self.indexOf(value) === index);
  }
  // Obtener los programas del canal por canal name
  getProgramsByChannelName(channelName) {
    return (this.getListaProgramas() || []).filter((program) => {
      try {
        if (!program)
          return false;
        const channelVal = program.channel;
        if (typeof channelVal === "string")
          return channelVal === channelName;
        if (channelVal && typeof channelVal === "object") {
          return channelVal.name === channelName || channelVal.id === channelName;
        }
        if (program.channel_id)
          return program.channel_id === channelName;
        return false;
      } catch (err) {
        return false;
      }
  ...
  getAllProgramsByCategory(category, channel) {
    return this.getListaProgramas().filter((program) => {
      if (program?.category?.value?.split(",")[0] === category && program.channel === channel)
        return program;
    });
  }
  // Otros métodos para obtener canales según el tipo (tdt, movistar, autonomico)
  getTDTCanales() {
    return this.listaCanales.filter((canal) => canal.type === "tdt");
  }
  getMovistarCanales() {
    return this.listaCanales.filter((canal) => canal.type === "movistar");
  }
  getAutonomicoCanales() {
    return this.listaCanales.filter((canal) => canal.type === "autonomico");
  }
  getDeportesCanales() {
    return this.listaCanales.slice(93, 103);
  }
  getCableCanales() {
    return this.listaCanales.filter((canal) => canal.type === "cable");
  }
  /**
   * Obtiene el resumen de todas las llamadas a APIs externas
   */
  getExternalApiCallSummary() {
    console.log(`\u{1F4CA} RESUMEN DE LLAMADAS A APIs EXTERNAS:`);
    console.log(`\u{1F525} Total de llamadas a TMDb API (pel\xEDculas y series): ${_TvGuideService.externalApiCallCounter}`);
    console.log(`\u{1F4BE} Cache de pel\xEDculas cargado: ${_TvGuideService.moviesCacheLoaded ? "S\xED" : "No"} (${_TvGuideService.moviesCache.length} elementos)`);
    console.log(`\u{1F4BE} Cache de series cargado: ${_TvGuideService.seriesCacheLoaded ? "S\xED" : "No"} (${_TvGuideService.seriesCache.length} elementos)`);
  }
  /**
   * Reinicia el contador de llamadas a APIs externas (TMDb)
   */
  resetExternalApiCallCounter() {
    _TvGuideService.externalApiCallCounter = 0;
    _TvGuideService.moviesCacheLoaded = false;
    _TvGuideService.seriesCacheLoaded = false;
    _TvGuideService.moviesCache = [];
    _TvGuideService.seriesCache = [];
    console.log(`\u{1F504} Contador de llamadas a APIs externas reiniciado y cache limpiado`);
  }
  /**
   * Obtiene el resumen completo de todas las llamadas de red
   */
  getCompleteCallSummary() {
    console.log(`
\u{1F4CA} ====== RESUMEN COMPLETO DE LLAMADAS DE RED ======`);
...
_TvGuideService.isLoadingMovies = false;
_TvGuideService.isLoadingSeries = false;
_TvGuideService.\u0275fac = function TvGuideService_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _TvGuideService)(\u0275\u0275inject(HttpService), \u0275\u0275inject(HomeDataService, 8));
};
_TvGuideService.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _TvGuideService, factory: _TvGuideService.\u0275fac, providedIn: "root" });
var TvGuideService = _TvGuideService;
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(TvGuideService, [{
    type: Injectable,
    args: [{
      providedIn: "root"
    }]
  }], () => [{ type: HttpService }, { type: HomeDataService, decorators: [{
    type: Optional
  }] }], null);
})();
...
@angular/router/fesm2022/router.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-MUKTTSZO.js.map
