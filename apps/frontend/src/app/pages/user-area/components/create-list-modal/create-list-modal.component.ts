...
@Component({
  ...
  template: `
    <div *ngIf="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        class="relative w-full max-w-sm bg-slate-900 border border-slate-800 rounded-2xl shadow-[0_20px_40px_rgba(0,0,0,0.45)]"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div class="p-5 space-y-4">
          ...
          <form [formGroup]="listForm" (ngSubmit)="onSubmit()" class="space-y-4">
            ...
            <div class="flex rounded-xl border border-slate-800 overflow-hidden">
              <label *ngFor="let opt of visibilityOptions" class="flex-1 cursor-pointer">
                ...
                <div class="text-center py-2.5 text-xs font-semibold uppercase tracking-wider border-r border-slate-800 last:border-r-0 peer-checked:bg-red-600/20 peer-checked:text-red-200 text-slate-400 transition-colors">
                  {{ opt.label }}
                </div>
              </label>
            </div>
            ...
            <div class="flex gap-3">
              <button
                type="button"
                (click)="close()"
                class="min-h-[44px] flex-1 px-4 rounded-xl border border-slate-700 text-slate-200 hover:text-white hover:border-slate-500 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500"
              >
                Cancelar
              </button>
              <button
